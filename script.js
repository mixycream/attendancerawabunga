...
// Logika Auto Clock-Out:
// - Berlaku untuk SEMUA divisi termasuk Security/Keamanan
// - Waktu auto-out = jam masuk + menit yang dikonfigurasi (global atau per divisi)
// - Misal setting 810 menit: jika masuk jam 06:00, auto-out di jam 19:30
...
async function autoClockOutForgotten() {
    ...
    try {
        ...
        const stuckWorkers = employees.map(emp => {
            ...
            // Hitung autoOutMs sesuai konfigurasi (global atau per divisi)
            ...
            if (now - autoOutAt > 7 * 24 * 3600 * 1000) return null;
...
function loadSettingsUI() {
    ...
    const orderedKeys = ["Helper Cook", "Cook", "Head Chef", "Packing", "Distribusi", "Kenek Distribusi", "Kebersihan", "Asisten Lapangan", "Admin Gudang", "Gudang", "Keamanan Shift 1", "Keamanan Shift 2", "Cuci Ompreng", "Leader Ompreng", "Leader Packing", "Leader Helper Cook", "Admin Yayasan", "Koordinasi Lapangan"];
    const allDivs = Array.from(new Set([...orderedKeys, ...Object.keys(appConfig.shifts || {})]));
    const filteredDivisions = allDivs; // Semua divisi termasuk Keamanan berlaku auto-out
...
// ═══════════════════════════════════════════════════════════════
// DATA READINESS SYSTEM — Solusi Internet Lambat
// Menampilkan status sinkronisasi dan retry QR otomatis
// ═══════════════════════════════════════════════════════════════

/**
 * _volDataReady: true jika employees[] sudah ada & cukup segar.
 * Di-set true setelah pre-fetch selesai atau cache valid ada.
 */
let _volDataReady  = false;
let _volFetching   = false;
let _volFetchedAt  = 0;   // timestamp terakhir data di-fetch
const _VOL_DATA_STALE_MS = 5 * 60 * 1000; // 5 menit = stale threshold
...
/**
 * _volPreFetch()
 * Silent background fetch saat volunteer page dibuka.
 * Mengisi employees[] & logs[] sebelum user sempat scan.
 */
...
/** Lightweight background refresh setelah halaman sudah ready */
async function _volFetchBackground() {
    if (_volFetching) return;
    _volFetching = true;
    try {
        const res  = await fetch(SCRIPT_URL + '?action=getData&_t=' + Date.now(), { signal: AbortSignal.timeout(25000) });
...
/**
 * _volShowRetryOverlay(show)
 * Menampilkan/menyembunyikan overlay "Memverifikasi…" di halaman scan QR.
 * Jika elemen belum ada, dibuat secara dinamis.
 */
function _volShowRetryOverlay(show) {
    let overlay = document.getElementById('volQRRetryOverlay');

    if (!overlay && show) {
        // Buat overlay dinamis di dalam volPageQR
        const parent = document.getElementById('volPageQR');
        if (!parent) return;

        overlay = document.createElement('div');
        overlay.id = 'volQRRetryOverlay';
        ...
        `;
        overlay.innerHTML = `
            <svg class="animate-spin" style="width:36px;height:36px;color:#10b981;" fill="none" viewBox="0 0 24 24">
                <circle style="opacity:0.25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path style="opacity:0.80" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
            </svg>
            <div style="text-align:center; padding: 0 24px;">
                <div style="color:#fff;font-size:13px;font-weight:800;letter-spacing:0.04em;">Memverifikasi QR&hellip;</div>
                <div style="color:rgba(148,163,184,0.85);font-size:11px;margin-top:4px;font-weight:500;">Mengambil data relawan, tunggu sebentar</div>
            </div>
        `;
...
let volActiveLivenessSteps = [];
let volLivenessStepIndex = 0;
...
function volGetStepIcon(stepKey) {
    switch (stepKey) {
        case 'face': return 'fas fa-user-circle';
        case 'blink': return 'fas fa-eye';
...
function volGetStepLabel(stepKey) {
    switch (stepKey) {
        case 'face': return 'Wajah';
        case 'blink': return 'Kedip';
...
function volStartLivenessCheck(video) {
    ...
    const verifCount = Math.floor(Math.random() * 3) + 2; // Returns 2, 3, or 4 checks
    ...
    volLivenessTimeout = setTimeout(() => {
        if (volLivenessActive && volLivenessStepIndex < volActiveLivenessSteps.length) {
            volStopLivenessCheck();
...
function volOnLivenessResults(results) {
    ...
    const verticalRatio = faceHeight > 0 ? (nose.y - forehead.y) / faceHeight : 0.55;
...
function volCompleteLiveness() {
    volLivenessStepIndex = volActiveLivenessSteps.length;
    volStopLivenessCheck();
    volUpdateLivenessUI();
