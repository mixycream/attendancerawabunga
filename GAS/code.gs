...
const DEFAULT_SHIFTS = {
  ...
  "Koordinasi Lapangan": { start: "07:00", end: "15:00" }
};
...
function getShiftsFromSheet() {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (div) {
      shifts[div] = { 
        start: formatSheetTime(data[i][1]), 
        end: formatSheetTime(data[i][2])
      };
    }
  }
...
function saveShiftsToSheet(shifts) {
  ...
  sheet.getRange(1, 1).setValue('Division');
  sheet.getRange(1, 2).setValue('Start');
  sheet.getRange(1, 3).setValue('End');
  sheet.getRange(1, 1, 1, 3).setFontWeight('bold');
...
function getData() {
  ...
  const config = { 
    ...
    allowMultipleIn: scriptProps.getProperty('allowMultipleIn') === 'true',
    enableLivenessCheck: scriptProps.getProperty('enableLivenessCheck') === 'true',
    enableSelfieOnly: scriptProps.getProperty('enableSelfieOnly') === 'true',
    geofenceLat: scriptProps.getProperty('geofenceLat') || "-6.21973",
    geofenceLng: scriptProps.getProperty('geofenceLng') || "106.87015",
    geofenceRadius: scriptProps.getProperty('geofenceRadius') || "15",
    lateTolerance: scriptProps.getProperty('lateTolerance') || "5",
    lateReasonThreshold: scriptProps.getProperty('lateReasonThreshold') || "25",
    lateWaThreshold: scriptProps.getProperty('lateWaThreshold') || "35",
    lateMaxThreshold: scriptProps.getProperty('lateMaxThreshold') || "60",
    earlyTolerance: scriptProps.getProperty('earlyTolerance') || "15",
    earlyReasonThreshold: scriptProps.getProperty('earlyReasonThreshold') || "20",
    earlyWaThreshold: scriptProps.getProperty('earlyWaThreshold') || "60",
    earlyMaxThreshold: scriptProps.getProperty('earlyMaxThreshold') || "120",
    adminWhatsApp: scriptProps.getProperty('adminWhatsApp') || "6282114806765",
    autoOutType: scriptProps.getProperty('autoOutType') || "global",
...
function addOrUpdateEmployee(data) {
  ...
  if (rowIndex > -1) {
    ...
    sheet.getRange(rowNum, 1).setValue(data.id); // Update ID in column 1
    ...
    // Update Logs sheet if ID changed
    if (data.oldId && String(data.oldId) !== String(data.id)) {
      const sheetLog = ss.getSheetByName("Logs");
      if (sheetLog && sheetLog.getLastRow() > 1) {
        const logLastRow = sheetLog.getLastRow();
        const logIdsRange = sheetLog.getRange(2, 3, logLastRow - 1, 1); // Column 3: EmpID
        const logIds = logIdsRange.getValues();
        const newLogIds = logIds.map(row => {
          if (String(row[0]) === String(data.oldId)) {
            return [data.id];
          }
          return [row[0]];
        });
        logIdsRange.setValues(newLogIds);
      }
    }
  } else {
...
function loadNutritionistPlan(data) {
  var ss = SpreadsheetApp.openById(SHEET_ID);
  var sheet = ss.getSheetByName('NutritionPlans');
  if (!sheet) {
    return response({ status: 'success', plan: null });
  }
  ...
  var rowData = null;
  if (sheet.getLastRow() > 1) {
    var ids = sheet.getRange(2, 1, sheet.getLastRow() - 1, sheet.getLastColumn()).getValues();
    for (var i = 0; i < ids.length; i++) {
      if ((userId && String(ids[i][0]).trim() === userId) || (username && String(ids[i][1]).trim().toLowerCase() === username.toLowerCase())) {
        foundRow = i + 2;
        rowData = ids[i];
  ...
  if (foundRow > 0 && rowData) {
    ...
    try {
      plan.ingredients = JSON.parse(rowData[9] || '[]');
    } catch (e) {
      plan.ingredients = [];
    }
...
function deleteNutritionistPlan(data) {
  var ss = SpreadsheetApp.openById(SHEET_ID);
  var sheet = ss.getSheetByName('NutritionPlans');
  if (!sheet) {
    return response({ status: 'success' });
...
function handleAttendance(data) {
  ...
  if (data.absentBy === 'Admin') {
    ...
    if (lastRow >= 2) {
      ...
      const isDuplicate = existingData.some(function(row) {
        // BUG FIX: row[0] dari getValues() adalah Date object (bukan string)
        // Harus diformat dulu sebelum dibandingkan dengan dateStr
  ...
  if (String(data.absentBy) !== 'Admin' && (String(data.type) === 'IN' || String(data.type) === 'PENDING')) {
    const scriptProps = PropertiesService.getScriptProperties();
    const allowMultipleIn = scriptProps.getProperty('allowMultipleIn') === 'true';
    const disableLate = scriptProps.getProperty('disableLate') === 'true';
    const disableBoth = scriptProps.getProperty('disableBoth') === 'true';
    const lateDisabled = disableLate || disableBoth;
    ...
    // Cek window 30 menit sebelum shift (hanya jika late tidak dinonaktifkan)
    if (!lateDisabled) {
      var empSheet = ss.getSheetByName("Employees");
...
function saveConfig(data) {
  ...
  if (data.customRoles) {
    var roles = data.customRoles;
    if (typeof roles !== 'string') roles = JSON.stringify(roles);
    scriptProps.setProperty('customRoles', roles);
  }
  if (data.autoOutDivisionsConfig) {
    var config = data.autoOutDivisionsConfig;
...
function saveFeatureSettings(data) {
  ...
  scriptProps.setProperty('allowMultipleIn', String(data.allowMultipleIn === true || data.allowMultipleIn === 'true'));
  scriptProps.setProperty('enableLivenessCheck', String(data.enableLivenessCheck === true || data.enableLivenessCheck === 'true'));
  scriptProps.setProperty('enableSelfieOnly', String(data.enableSelfieOnly === true || data.enableSelfieOnly === 'true'));
  scriptProps.setProperty('geofenceLat', String(data.geofenceLat || "-6.21973"));
  scriptProps.setProperty('geofenceLng', String(data.geofenceLng || "106.87015"));
  scriptProps.setProperty('geofenceRadius', String(data.geofenceRadius || "15"));
  scriptProps.setProperty('lateTolerance', String(data.lateTolerance || "5"));
  scriptProps.setProperty('lateReasonThreshold', String(data.lateReasonThreshold || "25"));
  scriptProps.setProperty('lateWaThreshold', String(data.lateWaThreshold || "35"));
  scriptProps.setProperty('lateMaxThreshold', String(data.lateMaxThreshold || "60"));
  scriptProps.setProperty('earlyTolerance', String(data.earlyTolerance || "15"));
  scriptProps.setProperty('earlyReasonThreshold', String(data.earlyReasonThreshold || "20"));
  scriptProps.setProperty('earlyWaThreshold', String(data.earlyWaThreshold || "60"));
  scriptProps.setProperty('earlyMaxThreshold', String(data.earlyMaxThreshold || "120"));
  scriptProps.setProperty('adminWhatsApp', String(data.adminWhatsApp || "6282114806765"));
  scriptProps.setProperty('autoOutType', String(data.autoOutType || "global"));
  scriptProps.setProperty('autoOutGlobalMinutes', String(data.autoOutGlobalMinutes || "240"));
  scriptProps.setProperty('autoOutDivisionsConfig', String(data.autoOutDivisionsConfig || "{}"));
  return response({ status: "success" });
}
